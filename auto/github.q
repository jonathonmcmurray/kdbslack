...
alrt:{[tl] /tl-table list - new issues, new state, new comments
  ...
  t:select distinct typ,
            first title,
         first comments,
               first pr,
            first state,
         first html_url by number from raze tl;
  m:{:$[`issue in x`typ;
         ("New ",$[first x`pr;"PR";"issue"]," #",string[x`number];x`html_url);      //new issue/PR
        `state`comment~x`typ;
         ("New comments on #",string[x`number],                                     //new comments
          " and state changed to ",string[x`state];x`html_url);                     //AND change of state
        `comment in x`typ;
         ("New comments on #",string[x`number];x`html_url);                         //new comments
        `state in x`typ;
         ("State of #",string[x`number]," changed to: ",string x`state;x`html_url); //change of state (open/closed)
         ("";x`html_rul)];}@'0!t;
...
upd getissues[];                                                                    //initial update to get starting state

\d .

.timer.add[`.gh.tm;`;00:05;1b]                                                      //add timer to check every 5 minutes
